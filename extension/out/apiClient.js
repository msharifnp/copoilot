...
class ApiClient {
    constructor() {
        ...
        this.http.interceptors.request.use((configReq) => {
            try {
     ...
     */
    async sendChatForm(text, files = [], sessionId, inlineFiles) {
        ...
        if (files.length === 0) {
            try {
                console.log("[RaaS] Sending as JSON (no files)");
                const { data } = await this.http.post(url, { text, user_id: "sharif_110", session_id: sessionId }, { headers: { "Content-Type": "application/json" } });
                return data;
            }
            catch (jsonError) {
