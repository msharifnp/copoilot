...
class ChatService:
    """Service for handling chat functionality with proper attachment handling"""
    ...
    @staticmethod
    async def build_messages_for_model(
        session_id: str,
        user_id: str,
        *,
        pairs_to_keep: int = 5,
        system_to_keep: int = 3,
        max_msg_chars: int = 12000
    ) -> list[dict]:
        """
        Compose LLM messages in chronological order:
        - base instruction
        - most recent system file notes ([FILE: …] OR older '[Attachment … uploaded at …]')
        - last N user/assistant turns (already stored, including current user)
        """
        def _truncate(msgs: list[dict], max_chars: int) -> None:
            for m in msgs:
                c = m.get("content")
                if isinstance(c, str) and len(c) > max_chars:
                    m["content"] = c[:max_chars] + "... [truncated]"

        # Oldest -> newest (your redis wrapper already returns chronological)
        history = await ChatService.get_chat_history(session_id, user_id) or []

        system_msgs = [m for m in history if m.get("role") == "system" and m.get("content")]
        dialog_msgs = [m for m in history if m.get("role") in ("user", "assistant") and m.get("content")]

        # Keep the most recent system notes
        keep_system = system_msgs[-system_to_keep:] if system_to_keep > 0 else []

        # Prefer the latest attachment-like note (works for both legacy and new formats)
        def _is_attachment(c: str) -> bool:
            return ("[FILE:" in c) or ("[Attachment" in c and "uploaded at" in c)

        pinned_attachment = next(
            (m for m in reversed(system_msgs) if _is_attachment(m.get("content", ""))),
            None
        )
        ...
        base_system = {
            "role": "system",
            "content": (
                "You are a helpful assistant. If there are system messages beginning with "
                "[FILE:…] (or legacy '[Attachment … uploaded at …]'), treat them as the "
                "user’s attached files and use them as the primary source when answering."
            )
        }

        messages: list[dict] = [base_system]

        # Latest attachment first (if any)
        if pinned_attachment:
            messages.append({"role": "system", "content": pinned_attachment["content"]})

        # Add remaining recent system notes (excluding the pinned one to avoid duplication)
        for m in keep_system:
            if not pinned_attachment or m.get("content") != pinned_attachment.get("content"):
                messages.append({"role": "system", "content": m["content"]})

        # Add recent dialog history (last N pairs = 2N messages)
        if pairs_to_keep > 0:
            messages.extend({"role": m["role"], "content": m["content"]} for m in dialog_msgs[-(pairs_to_keep * 2):])

        _truncate(messages, max_msg_chars)
        return messages


    @staticmethod
    async def clear_system_messages(session_id: str, user_id: str) -> bool:
        """Clear all system messages from the session cache to prevent contamination"""
        try:
            if redis_client.is_connected:
                # Get all messages
                messages = redis_client.get_chat_messages(session_id, user_id=user_id) or []
                ...
                # Reload only non-system messages
                if non_system_messages:
                    redis_client.load_chat_to_cache(session_id, non_system_messages, user_id=user_id)
                    
                logger.info(f"Cleared {len(messages) - len(non_system_messages)} system messages from session {session_id}")
                return True
        except Exception as e:
    ...
    @staticmethod
    async def mark_session_fresh_attachment(session_id: str, user_id: str) -> bool:
        """Mark a session as having fresh attachments to bypass cache"""
        try:
            if redis_client.is_connected:
                cache_key = f"fresh_attachment:{session_id}:{user_id}"
                redis_client.set_with_expiry(cache_key, "1", 300)  # 5 minutes
                return True
        except Exception as e:
            logger.error(f"Error marking fresh attachment: {e}")
        return False

    @staticmethod
    async def has_fresh_attachment(session_id: str, user_id: str) -> bool:
        """Check if session has fresh attachments"""
        try:
            if redis_client.is_connected:
                cache_key = f"fresh_attachment:{session_id}:{user_id}"
            ...
            logger.error(f"Error checking fresh attachment: {e}")
        return False

    @staticmethod
    async def store_message(session_id: str, user_id: str, role: str, content: str) -> bool:
        """Store message in Redis cache during live conversation."""
        try:
            if not redis_client.is_connected:
                return False
            ok = redis_client.add_chat_message(
    ...
    @staticmethod
    async def get_chat_history(session_id: str, user_id: str) -> List[Dict[str, Any]]:
        """Get chat history with Redis first, DB fallback"""
        try:
            if redis_client.is_connected:
                msgs = redis_client.get_chat_messages(session_id, user_id=user_id) or []
                return msgs
    ...
    async def generate_cache_key(
        text: str,
        context_msgs: list | None,
        session_id: str
    ) -> str:
        """Generate cache key that includes attachment context and session scope"""
        context_slice = (context_msgs or [])[-8:]
        
        # Extract system messages (attachments) separately for better cache isolation
        system_msgs = [m for m in context_slice if m.get("role") == "system"]
        dialog_msgs = [m for m in context_slice if m.get("role") in ("user", "assistant")]
        
        # Create hash-friendly structure
        compact = {
            "session_id": session_id,
            "text": text.strip().lower(),
            "system_count": len(system_msgs),
            "system_hash": hashlib.md5(
                json.dumps([m.get("content", "")[:1000] for m in system_msgs]).encode()
            ).hexdigest()[:8] if system_msgs else "",
            "dialog": [
                {"r": m.get("role"), "c": (m.get("content") or "")[:500]}
                for m in dialog_msgs[-4:]  # Last 2 exchanges only
            ]
        }
        
        blob = json.dumps(compact, sort_keys=True, ensure_ascii=False).encode()
        return "chat_response:" + hashlib.sha256(blob).hexdigest()
    ...
    @staticmethod
    async def cache_response(cache_key: str, response: str, ttl_seconds: int = 7200):
        """Cache the response (default 2 hours)"""
        try:
            if redis_client.is_connected:
                redis_client.set_with_expiry(cache_key, response, ttl_seconds)
        except Exception as e:
    ...
    async def process_chat_request(request: ChatRequest) -> ChatResponse:
        """
        Enhanced chat processing with proper attachment handling:
        1) Store current user turn first
        2) Build prompt including the recent system file notes
        3) Bypass cache read if fresh attachments exist
        """
        start = asyncio.get_event_loop().time()
        session_id, user_id = request.session_id, request.user_id

        # Config
        pairs_to_keep = int(os.getenv("CHAT_CONTEXT_PAIRS", "5"))
        system_to_keep = int(os.getenv("CHAT_SYSTEM_CONTEXT", "3"))
        max_msg_chars = int(os.getenv("CHAT_MAX_MSG_CHARS", "12000"))
        cache_ttl = int(os.getenv("CHAT_CACHE_TTL_SECONDS", "7200"))  # 2 hours

        # 1) Store the current user message FIRST so it appears in prompt
        await ChatService.store_message(session_id, user_id, "user", request.text or "")

        # 2) Build messages INCLUDING recent system file notes ([FILE:…] or legacy '[Attachment …]')
        model_msgs = await ChatService.build_messages_for_model(
            session_id=session_id,
            user_id=user_id,
            pairs_to_keep=pairs_to_keep,
            system_to_keep=system_to_keep,
            max_msg_chars=max_msg_chars
        )

        # 3) Cache logic
        has_fresh = await ChatService.has_fresh_attachment(session_id, user_id)
        use_cache = not has_fresh  # skip cache read if fresh attachments
        # Use the model context (before generating) to derive a cache key
        cache_key = await ChatService.generate_cache_key(request.text, model_msgs, session_id)
        cached_response = await ChatService.get_cached_response(cache_key) if use_cache else None

        from_cache = False
        model_used = "gemini-1.5-pro"

        if cached_response:
            logger.info(f"[Cache HIT] Returning cached response for: {request.text[:80]}...")
            ai_text = cached_response
            from_cache = True
            model_used = "cache"
        else:
            logger.info(f"[Cache MISS] Generating new response for: {request.text[:80]}...")
            ai_text = await ai_model.generate_chat_response(model_msgs)
            # Write-through cache even if fresh attachments; next turn will read
            await ChatService.cache_response(cache_key, ai_text, cache_ttl)

        # 4) Store assistant turn
        await ChatService.store_message(session_id, user_id, "assistant", ai_text)

        # 5) Clear fresh-attachment marker after we’ve used it
        if has_fresh and redis_client.is_connected:
            try:
                redis_client.delete(f"fresh_attachment:{session_id}:{user_id}")
            except Exception:
                pass
    ...
    async def flush_session_to_db(
        ...
        """Persist all messages from Redis to database"""
        try:
            messages: List[Dict[str, Any]] = []
            if redis_client.is_connected:
                messages = redis_client.get_chat_messages(session_id, user_id=user_id) or []
            ...
            if clear_cache and redis_client.is_connected:
                try:
                    redis_client.clear_chat_cache(session_id, user_id=user_id)
                    # Also clear fresh attachment marker
                    cache_key_fresh = f"fresh_attachment:{session_id}:{user_id}"
                    redis_client.delete(cache_key_fresh)
                except TypeError:
                    redis_client.clear_chat_cache(session_id)
    ...
    @staticmethod
    async def load_session_to_cache(session_id: str, user_id: str, ttl: Optional[int] = None) -> LoadChatResponse:
        """Load session from database to cache"""
        try:
            sess = await db_client.get_chat_session(session_id, user_id)
            if not sess:
                return LoadChatResponse(
            ...
            if isinstance(sess, str):
                try:
                    ...
                    raise TypeError("DB returned a string; expected an object. Check save_chat_session implementation.")
    ...
    @staticmethod
    async def list_user_sessions(user_id: str, limit: int = 30, offset: int = 0) -> List[Dict[str, Any]]:
        """List saved sessions from database for history/sidebar"""
        rows = []
        try:
            if hasattr(db_client, "fetch_all"):
                rows = await db_client.fetch_all(
...
class FileService:
    ...
    def extract_text_content(file_path: str, mime_type: Optional[str] = None) -> str:
        ...
        try:
            ...
            # Detect real MIME (override provided one)
            real_mime = FileService._detect_mime(file_path)
            ...
            # Structured handlers with MIME double-check
            if ext == ".csv":
                if not real_mime.startswith("text/") and "csv" not in real_mime:
                    return f"[Blocked: {file_path} is not a valid CSV file]"
                return FileService._extract_csv(file_path)
